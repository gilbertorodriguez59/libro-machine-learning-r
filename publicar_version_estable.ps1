...
$FinalPdf = Join-Path $DocsDir $PublicPdfName
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { Fail "Fallo la generacion del PDF." }
    ...
    if (-not $Pdf) { Fail "No se encontro ningun PDF generado." }
    ...
    if ($LASTEXITCODE -ne 0) { Fail "Fallo la generacion del HTML." }
    ...
    Write-Host ""
    Write-Host "4. Restaurando PDF en docs..." -ForegroundColor Cyan
    Copy-Item -LiteralPath $TempPdf -Destination $FinalPdf -Force

    $IndexHtml = Join-Path $DocsDir "index.html"
    if (-not (Test-Path -LiteralPath $IndexHtml)) { Fail "No existe docs\index.html." }
    if (-not (Test-Path -LiteralPath $FinalPdf)) { Fail "No existe docs\$PublicPdfName." }
    ...
    git add -A
    if ($LASTEXITCODE -ne 0) { Fail "Fallo git add." }

    git diff --cached --quiet
    if ($LASTEXITCODE -ne 0) {
        git commit -m "Actualizar sitio web y PDF de la version estable"
        if ($LASTEXITCODE -ne 0) { Fail "No se pudo crear el commit." }
    } else {
    ...
    git push
    if ($LASTEXITCODE -ne 0) { Fail "No se pudieron subir los cambios." }
